...
from SpamX.database.pmpermitdb import get_approved_users, pm_guard
import SpamX.database.pmpermitdb as SpamX
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await SpamX.set_block_message(SpamX.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await SpamX.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    pmpermit, pm_message, limit, block_message = await SpamX.get_pm_settings()
    await SpamX.allow_user(chat_id)
