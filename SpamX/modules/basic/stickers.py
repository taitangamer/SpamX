        ...
        if exist is not False:
            ...
            while limit in await get_response(message, client):
                ...
                if await get_response(message, client) == "Invalid pack selected.":
                    ...
                    await asyncio.sleep(2)
                    await um.edit(
                        f"**sᴛɪᴄᴋᴇʀ ᴀᴅᴅᴇᴅ sᴜᴄᴄᴇssғᴜʟʟʏ!**\n         🔥 **[ᴄʟɪᴄᴋ](https://t.me/addstickers/{packname})** 🔥\n**ᴛᴏ ᴜsᴇ sᴛɪᴄᴋᴇʀs**"
                    )
                    return
            await client.send_document("stickers", media_)
            await asyncio.sleep(2)
            if (
                ...
                await um.edit(
                    "**ғᴀɪʟᴇᴅ ᴛᴏ ᴀᴅᴅ sᴛɪᴄᴋᴇʀ, ᴜsᴇ @Stickers ʙᴏᴛ ᴛᴏ ᴀᴅᴅ ʏᴏᴜʀ sᴛɪᴄᴋᴇʀ.**"
                )
                return
            await client.send_message("Stickers", emoji_)
            await asyncio.sleep(2)
...
async def packinfo(client: Client, message: Message):
    ...
    output = f"""**sᴛɪᴄᴋᴇʀ ᴘᴀᴄᴋ ᴛɪᴛʟᴇ **: `{stickerset.set.title}`
**sᴛɪᴄᴋᴇʀ ᴘᴀᴄᴋ sʜᴏʀᴛ ɴᴀᴍᴇ **: `{stickerset.set.short_name}`
**sᴛɪᴄᴋᴇʀs ᴄᴏᴜɴᴛ **: `{stickerset.set.count}`
**ᴀʀᴄʜɪᴠᴇᴅ **: `{stickerset.set.archived}`
**ᴏғғɪᴄɪᴀʟ **: `{stickerset.set.official}`
**ᴍᴀsᴋs **: `{stickerset.set.masks}`
**ᴀɴɪᴍᴀᴛᴇᴅ **: `{stickerset.set.animated}`
**ᴇᴍᴏᴊɪs ɪɴ ᴘᴀᴄᴋ **: `{' '.join(emojis)}`
"""
