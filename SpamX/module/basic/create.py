...
async def create(client: Client, message: Message):
    if len(message.command) < 3:
        return await message.edit_text(
            message, f"**ᴛʏᴘᴇ .help ᴄʀᴇᴀᴛᴇ ɪғ ʏᴏᴜ ɴᴇᴇᴅ ʜᴇʟᴘ**"
        )
    group_type = message.command[1]
    split = message.command[2:]
    group_name = " ".join(split)
    xd = await message.edit_text("`ᴄʀᴇᴀᴛɪɴɢ...`")
    desc = "ᴡᴇʟᴄᴏᴍᴇ ᴛᴏ ᴍʏ " + ("Group" if group_type == "gc" else "Channel")
    if group_type == "gc":  # for supergroup
        _id = await client.create_supergroup(group_name, desc)
        link = await client.get_chat(_id["id"])
        await xd.edit(
            f"**sᴜᴄᴄᴇssғᴜʟʟʏ ᴄʀᴇᴀᴛᴇᴅ ᴛᴇʟᴇɢʀᴀᴍ ɢʀᴏᴜᴘ: [{group_name}]({link['invite_link']})**",
        ...
        link = await client.get_chat(_id["id"])
        await xd.edit(
            f"**sᴜᴄᴄᴇssғᴜʟʟʏ ᴄʀᴇᴀᴛᴇs ᴛᴇʟᴇɢʀᴀᴍ ᴄʜᴀɴɴᴇʟ: [{group_name}]({link['invite_link']})**",
            disable_web_page_preview=True,
        )
