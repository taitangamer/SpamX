        ...
        if dialog.chat.type in (enums.ChatType.GROUP, enums.ChatType.SUPERGROUP):
            ...
            if chat not in NB:
                try:
                    ...
                    await asyncio.sleep(0.3)
    await tex.edit_text(
        f"**sᴜᴄᴄᴇssғᴜʟʟʏ sᴇɴᴛ ᴍᴇssᴀɢᴇ ᴛᴏ** `{done}` **ɢʀᴏᴜᴘs, ᴄʜᴀᴛ, ғᴀɪʟᴇᴅ ᴛᴏ sᴇɴᴅ ᴍᴇssᴀɢᴇ ᴛᴏ** `{error}` **ɢʀᴏᴜᴘs**"
    )
...
async def gucast(client: Client, message: Message):
    ...
    await text.edit_text(
        f"**sᴜᴄᴄᴇssғᴜʟʟʏ sᴇɴᴛ ᴍᴇssᴀɢᴇ ᴛᴏ** `{done}` **ᴄʜᴀᴛ, ғᴀɪʟᴇᴅ ᴛᴏ sᴇɴᴅ ᴍᴇssᴀɢᴇ ᴛᴏ** `{error}` **ᴄʜᴀᴛ**"
    )
