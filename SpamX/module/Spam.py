...
async def delayspam(xspam: Client, e: Message): 
    ...
    if len(Rizoelop) == 2:
       ...
       if re.search(Owners.lower(), msg.lower()):
            return await e.reply_text(usage)("**Sorry !!** I can't Spam On @Venom_bolte_public's owner")
...
@Client.on_message(filters.user(SUDO_USERS) & filters.command(["love"], prefixes=HNDLR))
@Client.on_message(filters.me & filters.command(["love"], prefixes=HNDLR))
async def love(xspam: Client, e: Message):  
      ...
      if len(Rizoel) == 2:
          ...
          if int(id) in RiZoeLX:
                text = f"I can't love on @Venom_bolte_public's Owner"
          ...
          id = ok.id
          if int(id) in RiZoeLX:
                text = f"I can't love on @Venom_bolte_public Owner"
                await e.reply_text(text)
          elif int(id) == OWNER_ID:
                text = f"This guy is Owner Of this Bots."
                await e.reply_text(text)
      ...
      if LOGS_CHANNEL:
         try:
            await xspam.send_message(LOGS_CHANNEL, f"started Love By User: {e.from_user.id} \n\n On User: {mention} \n Chat: {e.chat.id} \n Counts: {counts}")
         except Exception as a:
             print(a)
             pass
...
async def raid(xspam: Client, e: Message):  
      ...
      if len(Rizoel) == 2:
          ...
          id = ok.id
          if int(id) in RiZoeLX:
                text = f"I can't raid on @Venom_bolte_public's Owner"
                await e.reply_text(text)
          elif int(id) == OWNER_ID:
                text = f"This guy is Owner Of this Bots."
                await e.reply_text(text)
...
async def fastspam(xspam: Client, e: Message):
    ...
    if len(Rizoel) == 2:
       ...
       msg = str(Rizoel[1])
       if re.search(Owners.lower(), msg.lower()):
            return await e.reply("**Sorry !!** I can't Spam On @Venom_bolte_public owner")
       if e.reply_to_message:
          reply_to_id = e.reply_to_message.message_id
          for _ in range(counts):
              await xspam.send_message(e.chat.id, msg, reply_to_message_id=reply_to_id)
